...
# --- 1. APP CONFIGURATION ---
st.set_page_config(page_title="Kaydiem Titan v23.1 | Global Sovereign Architect", layout="wide", page_icon="💎")
...
# --- 3. DATA COLLECTION ---
st.title("🏗️ Kaydiem Titan Supreme Engine v23.1")
...
# Prepare Global Variables to prevent NameErrors
...
# THEME CSS (Escaped braces {{ }} for CSS literals to fix SyntaxError)
theme_css = f"""
...
def generate_sovereign_layout(page_title, page_desc, content_body, is_home=False):
    v_tag = f'<meta name="google-site-verification" content="{gsc_tag_input}">' if (is_home and gsc_tag_input) else ""
    
    # RECTIFIED: Doubled braces for JavaScript literals
    dyn_script = ""
    ...
    <footer class="bg-slate-950 text-slate-400 py-24 px-10 border-t border-slate-900">
        <div class="max-w-[1440px] mx-auto grid md:grid-cols-4 gap-16 text-left">
            <div class="col-span-2">
                <h4 class="text-white text-3xl font-black mb-8 uppercase tracking-tighter uppercase leading-none">{biz_name}</h4>
...
t_html = "".join([f'<div class="p-10 bg-slate-50 rounded-[3rem] border border-slate-100 italic text-xl shadow-inner mb-8" style="color:var(--p)">"{t.split("|")[1].strip()}"<br><span class="font-black not-italic text-sm block mt-6 uppercase tracking-widest text-brand" style="color:var(--p)">— {t.split("|")[0].strip()} <span class="text-emerald-500 font-black ml-2 text-xs text-brand" style="color:var(--p)">● Partner</span></span></div>' for t in testi.splitlines() if "|" in t])
f_html = "".join([f'<details class="mb-6 bg-white p-6 rounded-2xl border border-slate-100 cursor-pointer shadow-sm"><summary class="font-black text-lg uppercase tracking-tight">{f.split("?")[0].strip()}?</summary><p class="mt-4 text-slate-600 leading-relaxed font-medium text-sm">{f.split("?")[1].strip()}</p></details>' for f in faqs.splitlines() if "?" in f])

# --- DYNAMIC STRUCTURAL DNAs ---
if layout_dna == "Industrial Titan":
    ...
    <section class="max-w-[1440px] mx-auto py-24 px-6 text-center border-b"><h2 class="section-title mb-20 uppercase tracking-tighter" style="color:var(--p)">Core Expertise</h2><div class="grid grid-cols-1 md:grid-cols-3 gap-10 text-left">{s_html}</div></section>
    <section id="inventory" class="py-32 px-6 max-w-[1440px] mx-auto text-center border-b"><h2 class="section-title mb-20 uppercase tracking-tighter" style="color:var(--p)">Exclusive Collection</h2><div id="live-data-container" class="grid grid-cols-1 md:grid-cols-4 gap-10 text-left"></div></section>
    ...
    <section class="p-10 max-w-[1440px] mx-auto grid md:grid-cols-4 gap-8">
        <div class="md:col-span-2 bg-slate-900 p-20 rounded-[4rem] text-white flex flex-col justify-center shadow-2xl text-left"><h2 class="text-7xl font-black mb-8 tracking-tighter uppercase leading-none">Authority Heritage</h2><p class="text-xl opacity-60 mb-10 italic">"Transforming landscapes since inception."</p><a href="about.html" class="btn-p w-fit no-underline">Full Story</a></div>
...
full_html_out = get_master_layout("Home", seo_d, idx_body, True)

if st.toggle("Activate Full Preview"):
    st.components.v1.html(full_html_out, height=800, scrolling=True)

if st.button("🚀 DEPLOY & DOWNLOAD THE WORLD'S BEST BUSINESS ASSET"):
    z_b = io.BytesIO()
    with zipfile.ZipFile(z_b, "a", zipfile.ZIP_DEFLATED, False) as z_f:
        z_f.writestr("index.html", full_html_out)
        z_f.writestr("about.html", get_master_layout("About Us", "History", f"<section class='max-w-7xl mx-auto py-32 px-6'><h1 class='legal-bold-title uppercase tracking-tighter text-brand' style='color:var(--p)'>About Our Heritage</h1><div class='text-xl md:text-2xl leading-relaxed text-slate-700 legal-text'>{about_txt}</div><img src='{img_g}' class='mt-20 w-full h-[600px] object-cover shadow-2xl' style='border-radius: var(--radius)'></section>"))
    ...
    st.success("💎 TITAN SOVEREIGN v22.1 READY. Fixed all Syntax and NameErrors.")
