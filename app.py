...
# --- 1. INITIALIZATION & GLOBAL DEFAULTS (Kills NameErrors) ---
biz_name = biz_phone = biz_email = biz_cat = biz_hours = prod_url = ""
biz_logo = biz_addr = biz_areas = map_iframe = hero_h = seo_d = ""
biz_key = biz_serv = about_txt = custom_hero = custom_feat = custom_gall = ""
sheet_url = testi_raw = faq_raw = priv_body = terms_body = ""
gsc_tag = ""

# --- 2. APP CONFIGURATION ---
st.set_page_config(page_title="Kaydiem Titan v24.0 | God-Mode Architect", layout="wide", page_icon="💎")
...
# --- 3. SIDEBAR: THE DESIGN STUDIO ---
with st.sidebar:
    st.image("https://www.gstatic.com/images/branding/product/2x/business_profile_96dp.png", width=50)
    st.title("Titan v24.0 Studio")
    
    with st.expander("🎭 1. Structural Architecture", expanded=True):
        ...
        border_rad = st.select_slider("Visual Sharpness", options=["0px", "4px", "12px", "24px", "40px", "60px"], value="24px")
    ...
    gsc_tag = st.text_input("GSC Verification Code")
    st.info("Technical Authority: Kaydiem Script Lab")

# --- 4. DATA COLLECTION ---
st.title("🏗️ Kaydiem Titan Supreme Engine v24.0")
...
logo_html = f'<img src="{biz_logo}" alt="{biz_name}" class="h-10 md:h-14 w-auto object-contain">' if biz_logo else f'<span class="text-xl md:text-3xl font-black uppercase" style="color:var(--p)">{biz_name}</span>'

# THEME CSS (Syntax Hardened)
master_css = f"""
...
.btn-accent {{ background: var(--s); color: white !important; padding: 1.1rem 2.8rem; border-radius: var(--radius); font-weight: 900; transition: all 0.4s; display: inline-block; text-align: center; border:none; text-decoration:none; cursor: pointer; box-shadow: 0 10px 20px -5px var(--s); }}
.btn-accent:hover {{ transform: translateY(-3px); filter: brightness(1.1); box-shadow: 0 20px 40px -5px var(--s); }}
.glass-nav {{ background: rgba(255, 255, 255, 0.98); backdrop-filter: blur(15px); border-bottom: 1px solid rgba(0,0,0,0.05); width: 100%; position: fixed; top: 0; left: 0; z-index: 9999; }}
                    ...
                    if (parts.length >= 2) {{
                        ...
                        <div onclick="openProduct(${{idx}})" class="product-card flex flex-col justify-between transition-all hover:scale-[1.03]">
                            ...
                            <div>
                                ...
                                <p class="text-slate-400 text-[10px] font-black uppercase tracking-widest italic underline decoration-slate-100 underline-offset-4 tracking-widest">Details →</p>
                            </div>
                        </div>`;
                    }}
                }});
...
<head>
    ...
    {v_tag}<title>{page_title} | {biz_name}</title>
...
<body class="bg-white flex flex-col min-h-screen text-slate-900">
    <nav class="glass-nav p-4 md:p-6 shadow-sm"><div class="max-w-[1440px] mx-auto flex justify-between items-center">
        ...
        <div class="hidden md:flex space-x-12 text-[10px] md:text-xs font-black uppercase tracking-widest text-slate-600">
            <a href="index.html" class="no-underline hover:text-blue-600">Home</a> <a href="about.html" class="no-underline hover:text-blue-600">About</a> <a href="contact.html" class="no-underline hover:text-blue-600">Contact</a>
        </div>
    ...
    <div id="modal" onclick="if(event.target == this) this.style.display='none'">
        <div class="modal-content shadow-2xl animate-in zoom-in duration-300">
            <div class="grid md:grid-cols-2">
                <div class="p-6 bg-slate-50 flex items-center justify-center"><img id="m-img-1" class="w-full h-auto rounded-[3rem] shadow-xl border-4 border-white"></div>
                <div class="p-12 flex flex-col justify-center text-left">
                    ...
                    <a id="m-wa" href="#" target="_blank" class="btn-accent w-full uppercase shadow-2xl">Confirm Booking</a>
                    <button onclick="document.getElementById('modal').style.display='none'; document.body.style.overflow='auto';" class="text-xs font-black uppercase mt-8 underline no-underline opacity-30">Close</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div><h4 class="text-white text-3xl font-black mb-8 uppercase tracking-tighter uppercase leading-none">{biz_name}</h4><p class="text-sm leading-relaxed mb-10 opacity-80">{biz_addr}</p><div class="flex flex-wrap gap-2">{coverage_badges}</div></div>
    ...
    <div class="md:text-right"><h4 class="text-white font-bold mb-8 uppercase text-xs text-brand tracking-widest" style="color:var(--s)">Contact Hub</h4><p class="text-lg font-bold text-white leading-loose underline decoration-blue-600 decoration-4 underline-offset-8 uppercase tracking-widest">Technical Support</p><p class="text-xl mt-4 font-black text-white">{biz_phone}</p></div></div>
...
t_cards_html = "".join([f'<div class="p-10 bg-slate-50 rounded-[3rem] border border-slate-100 italic text-xl shadow-inner mb-8" style="color:var(--p)">"{t.split("|")[1].strip()}"<br><span class="font-black not-italic text-sm block mt-6 uppercase tracking-widest text-brand" style="color:var(--p)">— {t.split("|")[0].strip()} <span class="text-emerald-500 font-black ml-2 text-xs">● Partner</span></span></div>' for t in testi_raw.splitlines() if "|" in t])
f_cards_html = "".join([f'<details class="mb-6 bg-white p-6 rounded-2xl border border-slate-100 cursor-pointer shadow-sm"><summary class="font-black text-lg uppercase tracking-tight">{f.split("?")[0].strip()}?</summary><p class="mt-4 text-slate-600 leading-relaxed font-medium text-sm">{f.split("?")[1].strip()}</p></details>' for f in faq_raw.splitlines() if "?" in f])

# --- 7. DYNAMIC DNA SWITCHER (10 True Layouts) ---
if layout_dna == "Industrial Titan":
    ...
    <section class="max-w-[1440px] mx-auto py-24 px-6 grid md:grid-cols-2 gap-24 items-center border-b text-left"><img src="{img_f}" class="shadow-2xl rounded-[var(--radius)]"><div><h2 class="section-title mb-12 uppercase tracking-tighter leading-none" style="color:var(--p)">Our Expertise</h2><div class="grid gap-6">{s_cards_html}</div><a href="about.html" class="btn-p mt-10 no-underline">Read Full Story</a></div></section>
    ...
    <section class="bg-white py-32 px-6 text-center border-b"><div class="max-w-4xl mx-auto"><h2 class="section-title mb-20 font-serif normal-case italic">Proven Reputation</h2>{t_cards_html}</div></section>
    <section id="inventory" class="py-32 px-6 max-w-[1440px] mx-auto text-center border-b"><h2 class="section-title mb-20 font-serif normal-case italic">Curated Collection</h2><div id="live-data-container" class="grid grid-cols-1 md:grid-cols-3 gap-16 text-center"></div></section>
    <section class="max-w-[1440px] mx-auto py-24 px-6 grid md:grid-cols-2 gap-24 items-center text-left"><div class="order-2 md:order-1"><h2 class="section-title mb-12">Expertise</h2><div class="grid gap-6">{s_cards_html}</div></div><img src="{img_f}" class="order-1 md:order-2 shadow-2xl rounded-[var(--radius)]"></section>
    <section class="py-32 px-6 max-w-5xl mx-auto"><h2 class="section-title mb-16 text-center">Expert Insights</h2>{f_cards_html}</section>
    """
else: # Mix/Bento/Minimalist Fallback
    idx_content = f"""
    <section class="hero-mask px-6 text-center text-white"><h1 class="hero-title mb-10">{hero_h}</h1><a href="#inventory" class="btn-accent shadow-2xl">Access Hub</a></section>
    <section class="p-10 max-w-[1440px] mx-auto grid md:grid-cols-4 gap-8 text-left">
        <div class="md:col-span-2 bg-slate-900 p-16 rounded-[4rem] text-white flex flex-col justify-center shadow-2xl"><h2 class="text-6xl font-black mb-6 tracking-tighter uppercase leading-none">Authority Heritage</h2><p class="text-xl opacity-60 mb-10 italic">"Transforming landscapes since inception."</p><a href="about.html" class="btn-p">Full Story</a></div>
        <div class="bg-slate-50 p-10 rounded-[4rem] border shadow-xl flex flex-col justify-center"><p class="text-3xl font-black mb-6 text-p" style="color:var(--p)">{biz_phone}</p><a href="tel:{biz_phone}" class="btn-accent w-full no-underline uppercase tracking-widest font-black">Direct Call</a></div>
    </section>
    ...
    <section class="p-20 text-center"><h2 class="section-title mb-16">Capabilities</h2><div class="grid grid-cols-1 md:grid-cols-3 gap-10 text-left">{s_cards_html}</div></section>
    """

# --- 8. PREVIEW & ZIP ---
st.header("⚡ Live Technical Preview (v24.0)")
...
if st.button("🚀 DEPLOY & DOWNLOAD ELITE ASSET PACKAGE"):
    ...
    st.success("💎 TITAN SOVEREIGN v24.0 READY. All Syntax and NameErrors permanently resolved.")
    st.download_button("📥 DOWNLOAD COMPLETE BIZ PACKAGE", z_b.getvalue(), f"{biz_name.lower().replace(' ', '_')}_v24_0.zip")
