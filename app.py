...
# --- 1. APP CONFIGURATION ---
st.set_page_config(page_title="Kaydiem Titan v22.1 | Infinity Architect", layout="wide", page_icon="💎")
...
# --- 3. DATA COLLECTION ---
st.title("🏗️ Kaydiem Titan Supreme Engine v22.1")
...
wa_final_link = f"https://wa.me/{wa_clean}?text=Hello%20{biz_name.replace(' ', '%20')}"
...
.hero-title {{ font-size: clamp(2rem, 8vw, 110px); text-shadow: 0 4px 20px rgba(0,0,0,0.4); }}
...
def get_master_layout(page_title, page_desc, content_html, is_h=False):
    ...
    if is_h and sheet_url:
                ...
                rows.forEach((parts, idx) => {{
                    if (parts.length >= 2) {{
                        ...
                        <div onclick="openProduct(${{idx}})" class="product-card flex flex-col justify-between transition-all hover:scale-[1.03]">
                            ...
                            <div><h3 class="text-xl font-black mb-2 uppercase" style="color:var(--p)">${{p.name}}</h3>
                            <p class="font-black text-2xl mb-4 text-s" style="color:var(--s)">${{p.price}}</p>
                            <p class="text-slate-400 text-[10px] font-black uppercase tracking-widest italic underline decoration-slate-100 underline-offset-4">Learn More →</p></div>
                        </div>`;
                    }}
                }});
        ...
        function openProduct(id) {{
            ...
            document.getElementById('m-wa').href = "{wa_final_link}" + encodeURIComponent(" (Interest in " + p.name + ")");
...
<body class="bg-white flex flex-col min-h-screen text-slate-900">
    ...
    <button onclick="closeModal()" class="text-xs font-black uppercase mt-8 underline no-underline opacity-30">Close Details</button></div></div></div></div>
...
if layout_dna == "Industrial Titan":
    ...
    <section class="max-w-[1440px] mx-auto py-24 px-6 grid md:grid-cols-2 gap-24 items-center border-b"><img src="{img_f}" class="shadow-2xl rounded-[var(--radius)]"><div><h2 class="section-title mb-12 uppercase tracking-tighter leading-none" style="color:var(--p)">Our Expertise</h2><div class="grid gap-6">{s_html}</div><a href="about.html" class="btn-p mt-10 no-underline">Read Full Story</a></div></section>
    <section id="inventory" class="py-32 px-6 max-w-[1440px] mx-auto text-center border-b"><h2 class="section-title mb-20 uppercase tracking-tighter" style="color:var(--p)">Live Inventory</h2><div id="live-data-container" class="grid grid-cols-1 md:grid-cols-4 gap-10 text-left"><p class="p-20 text-center text-slate-400 font-bold animate-pulse uppercase tracking-widest">Opening Data Hub...</p></div></section>
    <section class="py-32 px-6 max-w-7xl mx-auto text-center"><div class="grid md:grid-cols-2 gap-24 text-left"><div><h2 class="text-4xl font-black mb-16 uppercase tracking-tighter" style="color:var(--p)">Success</h2>{t_html}</div><div><h2 class="text-4xl font-black mb-16 uppercase tracking-tighter" style="color:var(--p)">Insights</h2>{"".join([f'<details class="mb-6 bg-white p-6 rounded-2xl border border-slate-100 cursor-pointer shadow-sm"><summary class="font-black text-lg uppercase tracking-tight">{f.split("?")[0].strip()}?</summary><p class="mt-4 text-slate-600 leading-relaxed font-medium text-sm">{f.split("?")[1].strip()}</p></details>' for f in faqs.splitlines() if "?" in f])}</div></div></section>
    """
    ...
    <section class="bg-white py-32 px-6 text-center border-b"><div class="max-w-4xl mx-auto"><h2 class="section-title mb-20 font-serif normal-case italic underline decoration-blue-600 underline-offset-8">Proven Reputation</h2>{t_html}</div></section>
    ...
    <section class="p-10 max-w-[1440px] mx-auto grid md:grid-cols-4 gap-8">
        <div class="md:col-span-2 bg-slate-900 p-16 rounded-[4rem] text-white flex flex-col justify-center shadow-2xl text-left"><h2 class="text-6xl font-black mb-6 tracking-tighter uppercase leading-none">Authority Heritage</h2><p class="text-xl opacity-60 mb-10 italic">"Transforming industrial landscapes since inception."</p><a href="about.html" class="btn-p w-fit no-underline">Full Story</a></div>
        <div class="bg-slate-50 p-10 rounded-[4rem] border shadow-xl flex flex-col justify-center"><p class="text-3xl font-black mb-6 text-p" style="color:var(--p)">{biz_phone}</p><a href="tel:{biz_phone}" class="btn-accent w-full no-underline uppercase tracking-widest text-[10px]">Connect</a></div>
...
st.header("⚡ Sovereign Technical Preview (v22.1)")
final_index_html = get_master_layout("Home", seo_d, idx_body, True)

if st.toggle("Activate Full Preview Mode"):
    st.components.v1.html(final_index_html, height=800, scrolling=True)

if st.button("🚀 DEPLOY & DOWNLOAD ELITE ASSET PACKAGE"):
    z_b = io.BytesIO()
    with zipfile.ZipFile(z_b, "a", zipfile.ZIP_DEFLATED, False) as z_f:
        z_f.writestr("index.html", final_index_html)
        z_f.writestr("about.html", get_master_layout("About Us", "History", f"<section class='max-w-7xl mx-auto py-32 px-6'><h1 class='legal-bold-title uppercase tracking-tighter text-brand' style='color:var(--p)'>Our Heritage</h1><div class='text-xl md:text-2xl leading-relaxed text-slate-700 legal-text'>{about_txt}</div><img src='{img_g}' class='mt-20 w-full h-[600px] object-cover shadow-2xl' style='border-radius: var(--radius)'></section>"))
        z_f.writestr("contact.html", get_master_layout("Contact", "Location", f"<section class='max-w-[1440px] mx-auto py-32 px-6 text-center'><h1 class='legal-bold-title uppercase tracking-tighter text-brand' style='color:var(--p)'>Technical Hub</h1><div class='grid md:grid-cols-2 gap-16 text-left'><div class='bg-slate-950 p-12 md:p-24 text-white' style='border-radius: var(--radius)'><p class='text-4xl font-black mb-8 text-white'>{biz_phone}</p><p class='text-2xl mb-12 opacity-80'>{biz_addr}</p><a href='tel:{biz_phone}' class='btn-accent w-full no-underline uppercase tracking-widest font-black'>Book Consultation</a></div><div class='rounded-[3rem] overflow-hidden border shadow-2xl bg-slate-100' style='min-height:300px'>{map_iframe}</div></div></section>"))
        z_f.writestr("privacy.html", get_master_layout("Privacy", "Legal", f"<div class='max-w-4xl mx-auto py-32 px-10'><h1 class='legal-bold-title uppercase tracking-tighter text-brand' style='color:var(--p)'>Privacy Policy</h1><div class='text-lg legal-text'>{priv_body}</div></div>"))
        z_f.writestr("terms.html", get_master_layout("Terms", "Legal", f"<div class='max-w-4xl mx-auto py-32 px-10'><h1 class='legal-bold-title uppercase tracking-tighter text-brand' style='color:var(--p)'>Terms & Conditions</h1><div class='text-lg legal-text'>{terms_body}</div></div>"))
        z_f.writestr("404.html", get_master_layout("404", "Not Found", "<div class='py-64 text-center'><h1 class='text-[120px] font-black uppercase text-slate-200 tracking-widest'>404</h1></div>"))
    ...
    st.success("💎 TITAN SOVEREIGN v22.1 READY. Fixed all NameErrors.")
