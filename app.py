...
# --- 1. APP CONFIGURATION ---
st.set_page_config(page_title="Kaydiem Titan v15.1 | Elite Architect", layout="wide", page_icon="💎")
...
st.title("🏗️ Kaydiem Titan Supreme v15.1")
...
# --- 3. THE SUPREME ENGINE V15.1 ---

if st.button("🚀 DEPLOY 1st CLASS ELITE ARCHITECTURE"):
    
    # SETUP GLOBAL VARIABLES BEFORE FUNCTION CALLS
    ...
    * {{ box-sizing: border-box; }}
    ...
    def get_layout(page_title, page_desc, content_body, is_home=False):
        ...
        # --- RECTIFIED DYNAMIC E-COM SCRIPT (Smarter Logic) ---
        dyn_script = ""
        if is_home and sheet_url:
            ...
            function openProduct(id) {{
                ...
                document.getElementById('m-wa').href = "{wa_url}" + encodeURIComponent(" (Interest in " + p.name + " - " + p.price + ")");
...
<body class="bg-white flex flex-col min-h-screen text-slate-900">
    ...
    # --- ARCHITECTURE DNA CONTENT SWITCH ---
    ...
    faq_html = "".join([f'<details class="mb-6 bg-white p-6 rounded-2xl border border-slate-100 cursor-pointer shadow-sm"><summary class="font-black text-lg uppercase tracking-tight">{f.split("?")[0].strip()}?</summary><p class="mt-4 text-slate-600 leading-relaxed font-medium text-sm">{f.split("?")[1].strip()}</p></details>' for f in faqs.splitlines() if "?" in f])
    ...
    if layout_dna == "Industrial Titan (Rugged)":
        ...
        <section class="hero-mask px-6 text-center text-white">
            <div class="max-w-[1200px] mx-auto">
                ...
                <a href="#inventory" class="btn-accent uppercase tracking-[0.4em] text-[10px] md:text-sm shadow-2xl" style="background:var(--p)">Direct Booking</a>
            </div>
        </section>
        <section class="max-w-[1440px] mx-auto py-24 px-6 text-center border-b">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-10 text-left">{"".join([f'<div class="bg-slate-50 p-12 rounded-[2.5rem] border shadow-xl"><h3 class="text-2xl font-black mb-4 uppercase">{s}</h3><p class="text-slate-500 text-sm italic uppercase font-bold">Verified Asset</p></div>' for s in biz_serv.splitlines()])}</div>
        </section>
        <section id="inventory" class="py-32 px-6 max-w-[1440px] mx-auto text-center border-b"><h2 class="section-title mb-20 uppercase tracking-tighter" style="color:var(--p)">Exclusive Offers</h2><div id="live-data-container" class="grid grid-cols-1 md:grid-cols-4 gap-8 text-left"></div></section>
        """
    elif layout_dna == "Classic Royal (Luxury)":
        idx_content = f"""
        <section class="hero-mask px-6 text-center text-white">
            <div class="max-w-[1200px] mx-auto">
                <h1 class="hero-title mb-10 normal-case italic font-serif" style="font-family: 'Playfair Display', serif;">{hero_h}</h1>
        ...
        <section class="bg-white py-32 px-6 text-center border-b">
            ...
            <div class="max-w-4xl mx-auto">{testi_html}</div>
        </section>
    ...
    st.success("💎 TITAN ELITE v15.1 DEPLOYED. Fixed NameError & Structural DNA Logic.")
