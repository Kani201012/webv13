...
# --- 1. APP CONFIGURATION ---
st.set_page_config(page_title="Kaydiem Titan v22.0 | Elite Sovereign Architect", layout="wide", page_icon="💎")
...
with st.sidebar:
    ...
    with st.expander("🎭 1. Layout DNA (Structural)", expanded=True):
        ...
        border_rad = st.select_slider("Visual Sharpness", options=["0px", "8px", "24px", "60px"], value="24px")
...
# --- 3. DATA COLLECTION ---
st.title("🏗️ Kaydiem Titan Supreme Engine v22.0")
...
# Prepare Global Logic Variables to avoid NameErrors inside functions
...
# THEME CSS (Escaped braces {{ }} for CSS literals)
theme_css = f"""
...
.hero-mask {{ background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.5)), url('{img_h}'); background-size: cover; background-position: center; min-height: 95vh; display: flex; align-items: center; justify-content: center; width: 100%; margin: 0; padding: 140px 20px 60px 20px; }}
...
def generate_sovereign_page(content_html, is_h=False):
    v_tag = f'<meta name="google-site-verification" content="{gsc_tag}">' if (is_h and gsc_tag) else ""
    
    # RECTIFIED DYNAMIC JS (Escaped Braces)
    dyn_script = ""
                    ...
                    if (parts.length >= 2) {{
                        ...
                        <div onclick="openProduct(${{idx}})" class="product-card flex flex-col justify-between transition-all hover:scale-[1.03]">
                            ...
                            <div>
                                ...
                                <p class="text-slate-400 text-[10px] font-black uppercase tracking-widest italic underline decoration-slate-100 underline-offset-4">Technical Specs →</p>
                            </div>
                        </div>`;
                    }}
                }});
...
<head>
    ...
    {v_tag}<title>{biz_name} | Enterprise Verified Asset</title>
...
<body class="bg-white flex flex-col min-h-screen text-slate-900">
    <nav class="glass-nav p-4 md:p-6 shadow-sm"><div class="max-w-[1440px] mx-auto flex justify-between items-center">
        ...
        <a href="tel:{biz_phone}" class="btn-accent" style="padding: 0.6rem 2rem; font-size: 11px;">CALL NOW</a>
    </div></nav>
        ...
        <div class="modal-content shadow-2xl animate-in zoom-in duration-300">
            <div class="grid md:grid-cols-2">
                ...
                <div class="p-12 flex flex-col justify-center text-left">
                    <h2 id="m-title" class="text-5xl font-black mb-4 uppercase text-p" style="color:var(--p)"></h2>
                    <p id="m-price" class="text-3xl font-black mb-8 text-s" style="color:var(--s)"></p>
                    <p id="m-desc" class="text-slate-600 mb-10 text-xl leading-relaxed"></p>
                    <a id="m-wa" href="#" target="_blank" class="btn-accent w-full text-lg py-5">CONFIRM BOOKING</a>
                </div>
            </div>
        </div>
    </div>
    <a href="{wa_final_url}" class="wa-float" target="_blank"><svg style="width:38px;height:38px" viewBox="0 0 24 24"><path fill="currentColor" d="M12.04 2c-5.46 0-9.91 4.45-9.91 9.91c0 1.75.46 3.45 1.32 4.95L2.05 22l5.25-1.38c1.45.79 3.08 1.21 4.74 1.21c5.46 0 9.91-4.45 9.91-9.91c0-2.65-1.03-5.14-2.9-7.01A9.816 9.816 0 0 0 12.04 2m.01 1.67c2.2 0 4.26.86 5.82 2.42a8.225 8.225 0 0 1 2.41 5.83c0 4.54-3.7 8.23-8.24 8.23c-1.48 0-2.93-.39-4.19-1.15l-.3-.17l-3.12.82l.83-3.04l-.2-.32a8.188 8.188 0 0 1-1.26-4.38c.01-4.54 3.7-8.24 8.25-8.24m-3.53 3.16c-.13 0-.35.05-.54.26c-.19.2-.72.7-.72 1.72s.73 2.01.83 2.14c.1.13 1.44 2.19 3.48 3.07c.49.21.87.33 1.16.43c.49.16.94.13 1.29.08c.4-.06 1.21-.5 1.38-.98c.17-.48.17-.89.12-.98c-.05-.09-.18-.13-.37-.23c-.19-.1-.1.13-.1.13s-1.13-.56-1.32-.66c-.19-.1-.32-.15-.45.05c-.13.2-.51.65-.62.78c-.11.13-.23.15-.42.05c-.19-.1-.8-.3-1.53-.94c-.57-.5-1.02-1.12-1.21-1.45c-.11-.19-.01-.29.09-.38c.09-.08.19-.23.29-.34c.1-.11.13-.19.19-.32c.06-.13.03-.24-.01-.34c-.05-.1-.45-1.08-.62-1.48c-.16-.4-.36-.34-.51-.35c-.11-.01-.25-.01-.4-.01Z"/></svg></a>
    ...
    <div class="col-span-2"><h4 class="text-white text-3xl font-black mb-8 uppercase tracking-tighter uppercase leading-none">{biz_name}</h4><p class="text-sm leading-relaxed mb-10 max-w-md opacity-80">{biz_addr}</p><div class="flex flex-wrap gap-2">{coverage_badges}</div></div>
    <div><h4 class="text-white font-bold mb-8 uppercase text-xs tracking-widest">Policy Hub</h4><ul class="space-y-4 text-sm font-bold uppercase list-none p-0"><li><a href="privacy.html" class="no-underline">Privacy</a></li><li><a href="terms.html" class="no-underline">Terms</a></li></ul></div>
    <div class="md:text-right"><h4 class="text-white font-bold mb-8 uppercase text-xs text-brand tracking-widest">Direct Connect</h4><p class="text-2xl font-black text-white">{biz_phone}</p><p class="text-sm mt-2">{biz_email}</p></div></div>
    <div class="text-center mt-20 opacity-20 text-[10px] uppercase font-black tracking-widest italic tracking-widest">Architected By <a href="https://www.kaydiemscriptlab.com" class="text-white underline">Kaydiem Script Lab</a></div></footer>{dyn_script}
</body></html>"""

# --- 5. COMPONENT BUILDERS (Restored & Modular) ---
s_cards_html = "".join([f'<div class="bg-slate-50 p-12 rounded-[2.5rem] border border-slate-100 shadow-xl hover:scale-[1.02] transition-transform"><h3 class="text-2xl font-black mb-4 uppercase" style="color:var(--p)">{s.strip()}</h3><p class="text-slate-500 text-sm font-bold uppercase tracking-tight italic">Verified technical solution for {biz_name}.</p></div>' for s in biz_serv.splitlines() if s.strip()])
t_cards_html = "".join([f'<div class="p-10 bg-slate-50 rounded-[3rem] border border-slate-100 italic text-xl shadow-inner mb-8" style="color:var(--p)">"{t.split("|")[1].strip()}"<br><span class="font-black not-italic text-sm block mt-6 uppercase tracking-widest text-brand" style="color:var(--p)">— {t.split("|")[0].strip()}</span></div>' for t in testi.splitlines() if "|" in t])

# --- 6. DYNAMIC DNA LOGIC (10 True Layouts) ---
if layout_dna == "Industrial Titan":
    idx_body = f"""
    <section class="hero-mask px-6 text-center text-white"><div class="max-w-[1200px] mx-auto"><h1 class="hero-title mb-10 uppercase tracking-tighter leading-none font-black">{hero_h}</h1><p class="text-lg md:text-3xl font-light mb-16 max-w-4xl mx-auto opacity-90 leading-tight">{seo_d}</p><a href="#inventory" class="btn-accent uppercase tracking-[0.4em] text-sm shadow-2xl">Discover Packages</a></div></section>
    <section class="max-w-[1440px] mx-auto py-24 px-6 grid md:grid-cols-2 gap-24 items-center border-b"><img src="{img_f}" class="shadow-2xl rounded-[var(--radius)]"><div><h2 class="section-title mb-12 uppercase tracking-tighter leading-none" style="color:var(--p)">Our Expertise</h2><div class="grid gap-6">{s_cards_html}</div><a href="about.html" class="btn-p mt-10 no-underline">Read Full Story</a></div></section>
    <section id="inventory" class="py-32 px-6 max-w-[1440px] mx-auto text-center border-b"><h2 class="section-title mb-20 uppercase tracking-tighter" style="color:var(--p)">Exclusive Offers</h2><div id="live-data-container" class="grid grid-cols-1 md:grid-cols-4 gap-10 text-left"></div></section>
    <section class="py-32 px-6 max-w-7xl mx-auto text-center"><div class="grid md:grid-cols-2 gap-24 text-left"><div><h2 class="text-4xl font-black mb-16 uppercase tracking-tighter" style="color:var(--p)">Success</h2>{t_cards_html}</div><div><h2 class="text-4xl font-black mb-16 uppercase tracking-tighter" style="color:var(--p)">Expert Insights</h2>{"".join([f'<details class="mb-6 bg-white p-6 rounded-2xl border border-slate-100 cursor-pointer shadow-sm"><summary class="font-black text-lg uppercase tracking-tight">{f.split("?")[0].strip()}?</summary><p class="mt-4 text-slate-600 leading-relaxed font-medium">{f.split("?")[1].strip()}</p></details>' for f in faqs.splitlines() if "?" in f])}</div></div></section>
    """
    ...
    <section class="bg-white py-32 px-6 text-center border-b"><div class="max-w-4xl mx-auto"><h2 class="section-title mb-20 font-serif normal-case italic underline decoration-blue-600 underline-offset-8">Proven Reputation</h2>{t_cards_html}</div></section>
    <section id="inventory" class="py-32 px-6 max-w-[1440px] mx-auto text-center"><h2 class="section-title mb-20 font-serif normal-case italic">The Collection</h2><div id="live-data-container" class="grid grid-cols-1 md:grid-cols-3 gap-16 text-center"></div></section>
    <section class="max-w-[1440px] mx-auto py-24 px-6 grid md:grid-cols-2 gap-24 items-center border-t border-slate-100 text-left"><div class="order-2 md:order-1"><h2 class="section-title mb-12">Expertise</h2><div class="grid gap-6">{s_cards_html}</div></div><img src="{img_f}" class="order-1 md:order-2 shadow-2xl rounded-[var(--radius)]"></section>
    """
else: # Bento / Modern Fallback
    idx_body = f"""
    <section class="hero-mask px-6 text-center text-white"><h1 class="hero-title mb-10">{hero_h}</h1><a href="#inventory" class="btn-accent shadow-2xl">Access Data Hub</a></section>
    <section class="p-10 max-w-[1440px] mx-auto grid md:grid-cols-4 gap-8">
        <div class="md:col-span-2 bg-slate-900 p-16 rounded-[4rem] text-white flex flex-col justify-center"><h2 class="text-6xl font-black mb-6 tracking-tighter uppercase leading-none">Authority Heritage</h2><p class="text-xl opacity-60 mb-10 italic">"Transforming industrial and luxury landscapes since inception."</p><a href="about.html" class="btn-p w-fit no-underline">Full Story</a></div>
        <div class="bg-slate-50 p-10 rounded-[4rem] border shadow-xl flex flex-col justify-center"><p class="text-3xl font-black mb-6 text-p" style="color:var(--p)">{biz_phone}</p><a href="tel:{biz_phone}" class="btn-accent w-full no-underline">Direct Call</a></div>
    </section>
...
# --- 7. PREVIEW & ZIP PACKAGING ---
st.header("⚡ Live Technical Preview")
full_index_html = generate_sovereign_page(idx_body, True)

if st.toggle("Show Real-Time Layout Preview"):
    st.components.v1.html(full_index_html, height=800, scrolling=True)

if st.button("🚀 DEPLOY & DOWNLOAD THE WORLD'S BEST BUSINESS ASSET"):
    z_b = io.BytesIO()
    with zipfile.ZipFile(z_b, "a", zipfile.ZIP_DEFLATED, False) as z_f:
        z_f.writestr("index.html", full_index_html)
        z_f.writestr("about.html", generate_sovereign_page(f"<section class='max-w-7xl mx-auto py-32 px-6'><h1 class='legal-bold-title uppercase tracking-tighter text-brand' style='color:var(--p)'>About Our Heritage</h1><div class='text-xl md:text-2xl leading-relaxed text-slate-700 legal-text'>{about_txt}</div><img src='{img_g}' class='mt-20 w-full h-[600px] object-cover shadow-2xl' style='border-radius: var(--radius)'></section>"))
        z_f.writestr("contact.html", generate_sovereign_page(f"<section class='max-w-[1440px] mx-auto py-32 px-6 text-center'><h1 class='legal-bold-title uppercase tracking-tighter text-brand' style='color:var(--p)'>Technical Hub</h1><div class='grid md:grid-cols-2 gap-16 text-left'><div class='bg-slate-950 p-12 md:p-24 text-white' style='border-radius: var(--radius)'><p class='text-4xl font-black mb-8 text-white'>{biz_phone}</p><p class='text-2xl mb-12 opacity-80'>{biz_addr}</p><a href='tel:{biz_phone}' class='btn-accent w-full no-underline uppercase tracking-widest font-black'>Book Consultation</a></div><div class='rounded-[3rem] overflow-hidden border shadow-2xl bg-slate-100' style='min-height:300px'>{map_iframe}</div></div></section>"))
        z_f.writestr("privacy.html", generate_sovereign_page(f"<div class='max-w-4xl mx-auto py-32 px-10'><h1 class='legal-bold-title uppercase tracking-tighter text-brand' style='color:var(--p)'>Privacy Policy</h1><div class='text-lg legal-text'>{priv_body}</div></div>"))
        z_f.writestr("terms.html", generate_sovereign_page(f"<div class='max-w-4xl mx-auto py-32 px-10'><h1 class='legal-bold-title uppercase tracking-tighter text-brand' style='color:var(--p)'>Terms & Conditions</h1><div class='text-lg legal-text'>{terms_body}</div></div>"))
        z_f.writestr("404.html", generate_sovereign_page(f"<div class='py-64 text-center'><h1 class='text-[120px] font-black uppercase text-slate-200 tracking-widest'>404</h1></div>"))
    ...
    st.success("💎 TITAN SOVEREIGN v22.0 DEPLOYED. Zero Defects Confirmed.")
    st.download_button("📥 DOWNLOAD PLATINUM ASSET", z_b.getvalue(), f"{biz_name.lower().replace(' ', '_')}_sovereign.zip")
