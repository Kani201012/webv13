...
# --- 1. APP CONFIGURATION ---
st.set_page_config(page_title="Kaydiem Titan v20.0 | Sovereign Architect", layout="wide", page_icon="💎")
...
with st.sidebar:
    ...
    with st.expander("🎭 1. Layout Architecture", expanded=True):
        layout_dna = st.selectbox("Select Site DNA", [
            ...
            "Minimalist", "Midnight Stealth", "Vivid Gradient", "Clean Health"
        ])
...
st.title("🏗️ Kaydiem Titan Supreme v20.0")
...
# THEME CSS GENERATOR (Escaped braces {{ }} for CSS literals)
theme_css = f"""
...
.btn-p {{ background: var(--p); color: white !border !important; padding: 0.9rem 2.2rem; border-radius: var(--radius); font-weight: 900; transition: all 0.4s; display: inline-block; text-align: center; border:none; text-decoration:none; text-transform: uppercase; font-size: 11px; }}
...
def get_layout(page_title, page_desc, content_body, is_home=False):
    ...
    # --- DYNAMIC E-COM SCRIPT (Escaped Braces) ---
    dyn_script = ""
    if is_home and sheet_url:
        ...
        function openProduct(id) {{
            ...
            document.getElementById('m-wa').href = "{wa_link}" + encodeURIComponent(" (Interest in " + p.name + ")");
...
t_html = "".join([f'<div class="p-10 bg-slate-50 rounded-[3rem] border italic text-xl shadow-inner mb-8" style="color:var(--p)">"{t.split("|")[1].strip()}"<br><span class="font-black not-italic text-sm block mt-6 uppercase tracking-widest text-brand" style="color:var(--p)">— {t.split("|")[0].strip()}</span></div>' for t in testi.splitlines() if "|" in t])
...
if layout_dna == "Industrial Titan":
    ...
    <section class="max-w-[1440px] mx-auto py-24 px-6 grid md:grid-cols-2 gap-24 items-center border-b"><img src="{img_f}" class="shadow-2xl rounded-[var(--radius)]"><div><h2 class="section-title mb-12 uppercase tracking-tighter leading-none">Core Expertise</h2><div class="grid gap-6">{serv_html}</div><a href="about.html" class="btn-p mt-10 no-underline" style="background:var(--p)">Read History</a></div></section>
    ...
    <section class="py-32 px-6 max-w-5xl mx-auto border-t"><h2 class="section-title mb-16 text-center">In-Depth FAQ</h2>{f_html}</section>
    """
else: # Default Mix / Bento DNA
    idx_content = f"""
    ...
    <section class="p-10 max-w-[1440px] mx-auto grid md:grid-cols-4 gap-8">
        <div class="md:col-span-2 bg-slate-900 p-16 rounded-[4rem] text-white flex flex-col justify-center"><h2 class="text-6xl font-black mb-6 tracking-tighter uppercase leading-none">Authority Heritage</h2><p class="text-xl opacity-60 mb-10 italic">"Transforming landscapes since inception."</p><a href="about.html" class="btn-p">Full Story</a></div>
    ...
    <section class="p-20"><h2 class="section-title mb-16 text-center">Technical Capabilities</h2><div class="grid md:grid-cols-3 gap-10 text-left">{serv_html}</div></section>
    <section class="py-32 px-6 bg-slate-50 border-y"><div class="max-w-7xl mx-auto grid md:grid-cols-2 gap-20"><div><h2 class="text-4xl font-black mb-10 uppercase">Trusted</h2>{t_html}</div><div><h2 class="text-4xl font-black mb-10 uppercase">FAQ</h2>{f_html}</div></div></section>
    """
...
if st.toggle("Activate Full Layout Preview"):
...
if st.button("🚀 DEPLOY & DOWNLOAD THE WORLD'S BEST BUSINESS ASSET"):
    ...
    st.success("💎 TITAN ELITE v20.0 READY.")
    st.download_button("📥 DOWNLOAD ENTERPRISE BIZ PACKAGE", z_b.getvalue(), f"{biz_name.lower().replace(' ', '_')}_v20.zip")
