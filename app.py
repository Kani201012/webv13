...
# --- 1. APP CONFIGURATION ---
st.set_page_config(page_title="Kaydiem Titan v19.0 | Zero-Error Architect", layout="wide", page_icon="💎")
...
st.title("🏗️ Kaydiem Titan Supreme v19.0")
...
with tabs[0]:
    c1, c2 = st.columns(2)
    with c1:
        biz_name = st.text_input("Business Name", "Urban Prime")
        biz_phone = st.text_input("Verified Phone", "+91 00000 00000")
        biz_email = st.text_input("Email", "contact@business.com")
    with c2:
        biz_cat = st.text_input("Category", "Industrial Solutions")
        biz_hours = st.text_input("Hours", "Mon-Sat: 09:00 - 18:00")
    ...
    biz_areas = st.text_area("Service Areas (Comma separated)", "Kolkata, Riyadh, Dammam")
    map_iframe = st.text_area("Map Embed HTML Code")

with tabs[1]:
    hero_h = st.text_input("Hero Headline", "Technical Sovereignty in the AI Era")
    seo_d = st.text_input("Meta Description", "Premium 17-point Google verified business asset.")
    biz_key = st.text_input("SEO Keywords")
    biz_serv = st.text_area("Services (One per line)")
    about_txt = st.text_area("Our Story (800+ Words)", height=300)
...
img_h = custom_hero if custom_hero else "https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?auto=format&fit=crop&q=80&w=1600"
...
# THEME ENGINE (Doubled braces {{ }} for CSS literals)
theme_css = f"""
:root {{ 
    --p: {p_color}; 
    --s: {s_color}; 
...
h1, h2, h3 {{ font-family: '{h_font}', sans-serif; font-weight: var(--weight); letter-spacing: var(--ls); text-transform: uppercase; line-height: 1.05; overflow-wrap: break-word; }}
...
def get_layout(title, desc, content, is_h=False):
    ...
    # --- RECTIFIED DYNAMIC E-COM SCRIPT (Doubled braces for JS) ---
    dyn_script = ""
...
<head>
    ...
    {v_tag}<title>{title} | {biz_name}</title><meta name="description" content="{desc}">
...
<body class="bg-white flex flex-col min-h-screen text-slate-900">
    ...
    <div class="p-12 flex flex-col justify-center text-left"><h2 id="m-title" class="text-4xl font-black mb-4 uppercase text-p" style="color:var(--p)"></h2><p id="m-price" class="text-3xl font-black mb-8 text-s" style="color:var(--s)"></p>
    <p id="m-desc" class="text-slate-600 mb-10 leading-relaxed text-lg legal-text"></p><a id="m-wa" href="#" target="_blank" class="btn-accent w-full uppercase tracking-widest">Confirm Selections</a>
    ...
    <footer class="bg-slate-950 text-slate-400 py-24 px-10 border-t border-slate-900"><div class="max-w-[1440px] mx-auto grid md:grid-cols-3 gap-16">
    <div><h4 class="text-white text-3xl font-black mb-8 uppercase">{biz_name}</h4><p class="text-sm leading-relaxed mb-10 opacity-80">{biz_addr}</p><div class="flex flex-wrap gap-2">{badges}</div></div>
    ...
    <div class="md:text-right"><h4 class="text-white font-bold mb-8 uppercase text-xs text-brand" style="color:var(--s)">Direct Connect</h4><p class="text-lg font-bold text-white leading-loose underline decoration-blue-600 decoration-4 underline-offset-8 uppercase tracking-widest">Technical Support</p><p class="text-xl mt-4 font-black text-white">{biz_phone}</p><p class="text-xs mt-2">{biz_email}</p></div></div>
    <div class="text-center mt-20 opacity-30 text-[10px] uppercase font-black tracking-widest italic tracking-widest">Architected By <a href="https://www.kaydiemscriptlab.com" class="text-white underline">Kaydiem Script Lab</a></div></footer>{dyn_script}
</body></html>"""

# --- COMPONENT LOGIC (Restored & Modular) ---
...
if layout_dna == "Industrial Titan (Rugged)":
    ...
    <section class="max-w-[1440px] mx-auto py-24 px-6 grid md:grid-cols-2 gap-24 items-center"><img src="{img_f}" class="shadow-2xl rounded-[var(--radius)]"><div><h2 class="section-title mb-12 uppercase tracking-tighter leading-none">Core Expertise</h2><div class="grid gap-6">{serv_html}</div><a href="about.html" class="btn-p mt-10 no-underline" style="background:var(--p)">Read History</a></div></section>
    ...
    <section class="bg-white py-32 px-6 text-center border-b"><div class="max-w-4xl mx-auto"><h2 class="section-title mb-20 font-serif normal-case italic">Proven Reputation</h2>{t_cards}</div></section>
    <section id="inventory" class="py-32 px-6 max-w-[1440px] mx-auto text-center"><h2 class="section-title mb-20 font-serif normal-case italic">Curated Collection</h2><div id="live-data-container" class="grid grid-cols-1 md:grid-cols-3 gap-16 text-center"></div></section>
    <section class="max-w-[1440px] mx-auto py-24 px-6 grid md:grid-cols-2 gap-24 items-center border-t border-slate-100"><div class="order-2 md:order-1"><h2 class="section-title mb-12">Expertise</h2><div class="grid gap-6">{serv_html}</div></div><img src="{img_f}" class="order-1 md:order-2 shadow-2xl rounded-[var(--radius)]"></section>
    <section class="py-32 px-6 max-w-5xl mx-auto"><h2 class="section-title mb-16 text-center">Expert Insights</h2>{f_cards}</section>
    """
else: # Default Bento / Modern Mix (The Bento Grid DNA)
    idx_content = f"""
    ...
    <section class="p-10 max-w-[1440px] mx-auto grid md:grid-cols-3 gap-8">
        <div class="md:col-span-2 bg-slate-900 p-16 rounded-[4rem] text-white flex flex-col justify-center"><h2 class="text-6xl font-black mb-6 tracking-tighter uppercase leading-none">Authority Heritage</h2><p class="text-xl opacity-60 mb-10 italic">"Transforming industrial and luxury landscapes since inception."</p><a href="about.html" class="btn-p">Full Story</a></div>
        <div class="bg-slate-50 p-10 rounded-[4rem] border shadow-xl"><h4 class="text-xs font-black uppercase mb-4 opacity-30 tracking-widest">Direct Contact</h4><p class="text-3xl font-black leading-none mb-6" style="color:var(--p)">{biz_phone}</p><a href="tel:{biz_phone}" class="btn-accent w-full">Connect</a></div>
    </section>
    ...
    <section class="p-20"><h2 class="section-title mb-16 text-center">Core Capabilities</h2><div class="grid md:grid-cols-3 gap-10">{serv_html}</div></section>
    <section class="py-32 px-6 bg-slate-50 border-y"><div class="max-w-7xl mx-auto grid md:grid-cols-2 gap-20"><div>{t_cards}</div><div>{f_cards}</div></div></section>
    """
...
if st.button("🚀 DEPLOY & DOWNLOAD ELITE PACKAGE"):
    ...
    st.success("💎 TITAN SUPREME v19.0 FINALIZED.")
    st.download_button("📥 DOWNLOAD COMPLETE BIZ PACKAGE", z_b.getvalue(), f"{biz_name.lower().replace(' ', '_')}_v19.zip")
