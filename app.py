...
st.set_page_config(page_title="Kaydiem Titan v15.0 | Elite Web Architect", layout="wide", page_icon="💎")
...
st.title("🏗️ Kaydiem Titan Supreme v15.0")
...
# --- 3. THE SUPREME TITAN ENGINE V15.0 ---
    ...
    def get_layout(page_title, page_desc, content_body, is_home=False):
        ...
        if is_home and sheet_url:
            ...
            function openProduct(id) {{
                ...
                document.getElementById('m-wa').href = "https://wa.me/{wa_clean}?text=" + encodeURIComponent("Interest: " + p.name + " (" + p.price + ")");
...
<body class="bg-white flex flex-col min-h-screen text-slate-900">
    ...
    <footer class="bg-slate-950 text-slate-400 py-24 px-10 border-t border-slate-900">
        <div class="max-w-[1440px] mx-auto grid md:grid-cols-4 gap-16 text-left">
            <div class="col-span-2">
                ...
                <p class="text-[10px] mt-10 opacity-30 uppercase font-black tracking-widest italic tracking-widest">Built by <a href="https://www.kaydiemscriptlab.com" class="text-white underline">Kaydiem Script Lab</a></p>
            </div>
            <div><h4 class="text-white font-bold mb-8 uppercase text-xs">Legal</h4><ul class="space-y-4 text-sm font-bold uppercase"><li><a href="privacy.html" class="hover:text-white transition no-underline">Privacy</a></li><li><a href="terms.html" class="hover:text-white transition no-underline">Terms</a></li></ul></div>
            <div><h4 class="text-white font-bold mb-8 uppercase text-xs">Direct Support</h4><p class="text-lg font-bold text-white leading-loose underline decoration-blue-600 decoration-4 underline-offset-8 uppercase tracking-widest">Support Hub</p><p class="text-xl mt-4 font-black text-white">{biz_phone}</p><p class="text-xs mt-2">{biz_email}</p></div>
        </div>
    </footer>
    {dyn_script}
</body></html>"""
    ...
    if layout_dna == "Industrial Titan (Rugged)":
        ...
        <section id="inventory" class="py-32 px-6 max-w-[1440px] mx-auto text-center border-t">
            <h2 class="section-title mb-20 uppercase tracking-tighter">Exclusive Collection</h2>
            <div id="live-data-container" class="grid grid-cols-1 md:grid-cols-4 gap-10 text-left"><p class="p-20 text-center text-slate-400 font-bold animate-pulse">Syncing Data...</p></div>
        </section>
        """
        ...
        <section class="hero-mask px-6 text-center text-white">
            <div class="max-w-[1200px] mx-auto">
                <h1 class="hero-title mb-10 tracking-tighter leading-none font-serif normal-case italic">{hero_h}</h1>
                <p class="text-lg md:text-3xl font-light mb-16 max-w-4xl mx-auto opacity-90 leading-tight">{seo_d}</p>
                <a href="#inventory" class="btn-accent tracking-widest text-[10px] md:text-sm">Explore Showroom</a>
            </div>
        </section>
        <section id="inventory" class="py-32 px-6 max-w-[1440px] mx-auto text-center">
            <h2 class="section-title mb-20 tracking-tighter font-serif normal-case italic">Curated Selection</h2>
            <div id="live-data-container" class="grid grid-cols-1 md:grid-cols-3 gap-16 text-center"></div>
        </section>
        <section class="bg-slate-50 py-32 px-6 text-center">
            <div class="max-w-4xl mx-auto">
                <h2 class="text-5xl font-black mb-12" style="color:var(--p)">About the Legacy</h2>
                <p class="text-xl text-slate-600 mb-10 italic">{about_txt[:300]}...</p>
        ...
        idx_content = f"""
        <section class="hero-mask px-6 text-center text-white">
            <h1 class="hero-title mb-10">{hero_h}</h1>
            <a href="#inventory" class="btn-accent">View Data Hub</a>
        </section>
        ...
        <section class="p-20"><div class="grid md:grid-cols-3 gap-10">{"".join([f'<div class="bg-slate-100 p-10 rounded-3xl h-64 flex items-center justify-center text-center font-black"><h3>{s}</h3></div>' for s in biz_serv.splitlines()])}</div></section>
        """
    ...
    st.success("💎 TITAN ELITE v15.0 DEPLOYED. True Multi-Layout Engine active.")
    st.download_button("📥 DOWNLOAD COMPLETE BIZ PACKAGE", z_b.getvalue(), f"{biz_name.lower().replace(' ', '_')}_v15_0.zip")
