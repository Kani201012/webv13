...
# --- 1. APP CONFIGURATION ---
st.set_page_config(page_title="Kaydiem Titan v16.0 | Universal Sovereign", layout="wide", page_icon="💎")
...
with st.sidebar:
    ...
    with st.expander("🎭 1. Layout DNA", expanded=True):
        layout_dna = st.selectbox("Design Strategy", [
            "Industrial Titan", "Classic Royal", "Glass-Tech", 
            "Bento Modern", "Brutalist", "Corporate Elite", 
            "Minimalist", "Midnight Stealth", "Vivid Creative", "Architectural"
        ])
        p_color = st.color_picker("Primary Brand Color", "#0F172A")
        s_color = st.color_picker("Accent/CTA Color", "#2563EB")
        border_rad = st.select_slider("Corner Sharpness", options=["0px", "4px", "12px", "40px", "60px"], value="12px")

    with st.expander("✍️ 2. Typography Studio", expanded=True):
        h_font = st.selectbox("Heading Font", ["Montserrat", "Playfair Display", "Oswald", "Syncopate", "Inter", "Space Grotesk"])
        b_font = st.selectbox("Body Font", ["Inter", "Roboto", "Lora", "Work Sans", "Open Sans"])
...
tabs = st.tabs(["📍 Identity", "🏗️ Content", "🖼️ Images", "⚡ Live E-com", "🌟 Trust", "⚖️ Legal"])

with tabs[0]:
    c1, c2 = st.columns(2)
    with c1:
        biz_name = st.text_input("Business Name (NAP)", "Kaydiem Script Lab")
        biz_phone = st.text_input("Verified Phone", "+91 95177 02337")
        biz_email = st.text_input("Business Email", "info@kaydiemscriptlab.com")
    with c2:
        biz_cat = st.text_input("Category", "Technical Web Lab")
        biz_hours = st.text_input("Hours", "Open 24 Hours")
        prod_url = st.text_input("Production URL", "https://www.kaydiemscriptlab.com/")
    biz_logo = st.text_input("Logo URL (Transparent PNG)")
    biz_addr = st.text_area("Full Maps Physical Address")
    biz_areas = st.text_area("Service Areas (Comma separated)", "Kolkata, Riyadh, Lusaka, Khartoum")
    map_iframe = st.text_area("Map Embed HTML Code")

with tabs[1]:
    hero_h = st.text_input("Hero Headline", "Stop Building Websites. Start Deploying Technical Dominance.")
...
with tabs[3]:
    ...
    sheet_url = st.text_input("Published CSV Link (Use Pipe | Separator)")
...
# --- 3. THE SUPREME ENGINE V16.0 ---

if st.button("🚀 GENERATE FLAWLESS SOVEREIGN ASSET"):
    
    # 3.1 Setup Images
    img_h = custom_hero if custom_hero else "https://images.unsplash.com/photo-1550751827-4bd374c3f58b?q=80&w=1600"
    img_f = custom_feat if custom_feat else "https://images.unsplash.com/photo-1460925895917-afdab827c52f?q=80&w=800"
    img_g = custom_gall if custom_gall else "https://images.unsplash.com/photo-1504384308090-c894fdcc538d?q=80&w=1600"
    
    # 3.2 Global Variable Sync
    wa_clean = biz_phone.replace(" ", "").replace("+", "")
    wa_url = f"https://wa.me/{wa_clean}?text=Hello%20Kaydiem%20Lab"
    area_list_clean = [a.strip() for a in biz_areas.split(",")]
    s_areas_json = json.dumps(area_list_clean)
    
    # 3.3 The CSS Theme Engine
    theme_css = f"""
    ...
    * {{ box-sizing: border-box; }}
    ...
    .hero-title {{ font-size: clamp(1.8rem, 8vw, 100px); text-shadow: 0 4px 20px rgba(0,0,0,0.4); }}
    ...
    .glass-nav {{ background: rgba(255, 255, 255, 0.98); backdrop-filter: blur(15px); border-bottom: 1px solid rgba(0,0,0,0.05); position: fixed; top: 0; left:0; width: 100%; z-index: 9999; }}
    ...
    def get_layout_template(title, desc, content_body, is_home=False):
                ...
                try {{
                    ...
                    rows.forEach((parts, idx) => {{
                        if (parts.length >= 2) {{
                            ...
                            container.innerHTML += `
                            <div onclick="openProduct(${{idx}})" class="bg-white p-8 rounded-[2.5rem] border shadow-xl cursor-pointer hover:scale-[1.03] transition-all">
        ...
        logo_html_gen = f'<img src="{biz_logo}" alt="{biz_name}" class="h-10 md:h-14 w-auto object-contain">' if biz_logo else f'<span class="text-xl md:text-3xl font-black tracking-tighter uppercase" style="color:var(--p)">{biz_name}</span>'
...
<body class="bg-white flex flex-col min-h-screen text-slate-900">
    <nav class="glass-nav p-4 md:p-6 shadow-sm">
        <div class="max-w-[1440px] mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
            <a href="index.html" class="no-underline">{logo_html_gen}</a>
        ...
        <div class="modal-content shadow-2xl animate-in zoom-in duration-300">
            <div class="grid md:grid-cols-2">
                ...
                <div class="p-12 flex flex-col justify-center text-left">
                    ...
                    <a id="m-wa" href="#" target="_blank" class="btn-accent w-full uppercase tracking-widest shadow-2xl">Consult Now</a>
            ...
            <div class="col-span-2">
                ...
                <div class="bg-slate-900/50 p-6 border border-slate-800 rounded-3xl">
                    ...
                    <div class="flex flex-wrap gap-2">
                        {"".join([f'<span class="bg-slate-800 text-[10px] px-3 py-1 rounded-full uppercase font-bold text-white tracking-widest border border-slate-700">{a}</span>' for a in area_list_clean])}
                    </div>
                </div>
    ...
    # --- ENHANCED INDEX CONTENT (Switch Based on DNA) ---
    ...
    dynamic_section = f"""<section id="inventory" class="py-32 px-6 max-w-[1440px] mx-auto text-center border-b"><h2 class="section-title mb-20 uppercase tracking-tighter" style="color:var(--p)">Exclusive Collection</h2><div id="live-data-container" class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-8 text-left"><p class="p-20 text-center text-slate-400 font-bold animate-pulse uppercase tracking-widest">Establishing Data Bridge...</p></div></section>""" if sheet_url else ""
    ...
    <section class="hero-mask px-6 text-center text-white">
        <div class="max-w-[1200px] mx-auto">
            ...
            <a href="tel:{biz_phone}" class="btn-accent uppercase tracking-[0.4em] text-[10px] md:text-sm shadow-2xl">Connect Directly</a>
        </div>
    </section>
    <section class="max-w-[1440px] mx-auto py-24 px-6 text-center border-b">
        <h2 class="section-title mb-20 uppercase tracking-tighter" style="color:var(--p)">Core Capabilities</h2>
    ...
    <section class="bg-slate-50 py-32 px-6 border-y text-left">
        <div class="max-w-[1440px] mx-auto grid md:grid-cols-2 gap-24 items-center">
            ...
            <div>
                ...
                <p class="text-2xl text-slate-600 mb-12 leading-relaxed italic">"Supplying the engineering foundation for the 2026 urban landscape. Precision execution, certified safety, and direct owner oversight."</p>
    ...
    with zipfile.ZipFile(z_b, "a", zipfile.ZIP_DEFLATED, False) as z_f:
        z_f.writestr("index.html", get_layout_template("Home", seo_d, idx_content, True))
        z_f.writestr("about.html", get_layout_template("About Us", "History", f"<section class='max-w-7xl mx-auto py-32 px-6'><h1 class='legal-bold-title uppercase tracking-tighter text-brand' style='color:var(--p)'>About Our Heritage</h1><div class='text-xl md:text-2xl leading-relaxed text-slate-700 legal-text'>{about_txt}</div><img src='{img_g}' class='mt-20 w-full h-[600px] object-cover shadow-2xl' style='border-radius: var(--radius)'></section>"))
        z_f.writestr("contact.html", get_layout_template("Contact", "Location", f"<section class='max-w-[1440px] mx-auto py-32 px-6 text-center'><h1 class='legal-bold-title uppercase tracking-tighter text-brand' style='color:var(--p)'>Technical Hub</h1><div class='grid md:grid-cols-2 gap-16 text-left'><div class='bg-slate-950 p-12 md:p-24 text-white' style='border-radius: var(--radius)'><p class='text-4xl font-black mb-8 text-white'>{biz_phone}</p><p class='text-2xl mb-12 opacity-80'>{biz_addr}</p></div><div class='rounded-[3rem] overflow-hidden border shadow-2xl bg-slate-100' style='min-height:300px'>{map_iframe}</div></div></section>"))
        z_f.writestr("privacy.html", get_layout_template("Privacy", "Legal", f"<div class='max-w-4xl mx-auto py-32 px-10'><h1 class='legal-bold-title uppercase tracking-tighter text-brand' style='color:var(--p)'>Privacy Policy</h1><div class='text-lg legal-text'>{priv_body}</div></div>"))
        z_f.writestr("terms.html", get_layout_template("Terms", "Legal", f"<div class='max-w-4xl mx-auto py-32 px-10'><h1 class='legal-bold-title uppercase tracking-tighter text-brand' style='color:var(--p)'>Terms & Conditions</h1><div class='text-lg legal-text'>{terms_body}</div></div>"))
        z_f.writestr("404.html", get_layout_template("404", "Not Found", "<div class='py-64 text-center'><h1 class='text-[120px] font-black uppercase text-slate-200 tracking-widest'>404</h1></div>"))
    ...
    st.success("💎 TITAN SUPREME v16.0 DEPLOYED. Technical Scope Rectified.")
